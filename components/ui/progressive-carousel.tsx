...
export const ProgressSlider: FC<ProgressSliderProps> = ({
  ...
  useEffect(() => {
    const getChildren = React.Children.toArray(children).find(
      (child) => (child as React.ReactElement<SliderContentProps>).type === SliderContent
  ...
  const goToSlide = useCallback((newValue: string) => {
    setActive(newValue);
  ...
  const animate = useCallback((now: number) => {
    ...
    if (isFastForwardRef.current) {
      const newProgress = startProgressRef.current + (100 - startProgressRef.current) * timeFraction;
  ...
  const goToPrev = useCallback(() => {
    const currentIndex = sliderValues.indexOf(active);
    const prevIndex = currentIndex === 0 ? sliderValues.length - 1 : currentIndex - 1;
  ...
  return (
    <ProgressSliderContext.Provider
      value={{ active, progress, handleButtonClick, vertical, goToNext, goToPrev, sliderValues }}
    >
      ...
      {sliderValues.map((value) => {
        ...
        return (
          <button
            ...
            onClick={() => handleButtonClick(value)}
            className={cn(
              "h-1 flex-1 rounded-full overflow-hidden bg-foreground/20 transition-all",
              indicatorClass
            )}
          >
            <div
